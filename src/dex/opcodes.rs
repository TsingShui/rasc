...
/// Width in the low byte and the kind mask in the high byte, so the instruction loop
/// does one dependent load per instruction instead of two from different cache lines.
///
/// The loop is latency-bound (`code[pc]` -> table -> `pc`), so removing a load from
/// that chain is worth more than the table being twice as large: 512 bytes still fit
/// in four cache lines.
pub(super) const OPCODE_INFO: [u16; 256] = {
    let mut table = [0u16; 256];
    let mut opcode = 0;
    while opcode < 256 {
        table[opcode] = OPCODE_UNITS[opcode] as u16 | ((OPCODE_KINDS[opcode] as u16) << 8);
        opcode += 1;
    }
    table
};
...
mod tests {
    ...
    #[test]
    fn fused_opcode_table_agrees_with_the_two_it_packs() {
        for opcode in 0..=u8::MAX {
            let info = OPCODE_INFO[opcode as usize];
            assert_eq!(info as u8, OPCODE_UNITS[opcode as usize], "0x{opcode:02x}");
            assert_eq!(
                (info >> 8) as u8,
                OPCODE_KINDS[opcode as usize],
                "0x{opcode:02x}"
            );
        }
    }
