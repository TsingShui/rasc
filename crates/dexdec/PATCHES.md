...
| 6 | `src/timing.rs`: a `crate::timing::Instant` shim, used by every former `std::time::Instant::now()` call site; it is a re-export of `std::time::Instant` | Upstream keeps the module so a target can substitute a clock. The zero-duration stub this carried for `wasm32-unknown-unknown` is gone with that target: rasc builds native and `wasm32-wasip1`, and both have a real clock |
