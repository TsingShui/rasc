...
pub struct DexFileReader {
    ...
    /// Immutable class directory used by every on-demand materialization and by
    /// the lazy lexical-ownership lookups. Ordered so a nested class's
    /// `<outer>$` children can be ranged without decoding annotations.
    class_locations: BTreeMap<String, (usize, usize)>,
    ...
    /// Reverse of the lexical-ownership map, built on first use. A class's
    /// children are not derivable from its own annotations: an obfuscated app can
    /// point `EnclosingClass` at a descriptor with no `$` relationship
    /// (`LX/0108;` inside `LX/0107;`), so the only exact definition is the inverse
    /// of the global parent map. `children_of` fills this once and later queries
    /// hit the cache.
    nested_children: OnceLock<HashMap<String, Vec<String>>>,
}
...
impl DexFileReader {
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> DexResult<Self> {
        ...
        {
            let _ = path_ref;
            Err(DexError::IoError(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "APK input is disabled in this build (feature `apk` off); pass DEX bytes",
            )))
        }
    }
    ...
    pub fn class_names(&self) -> Vec<String> {
        self.dex_files
            .iter()
            .flat_map(|d| d.get_classes_names())
            .map(str::to_string)
            .collect()
    }
    ...
    /// the class body. Decodes only the queried classes' annotations.
    pub fn lexical_parent(&self, class_name: &str) -> Option<String> {
        if let Some(parent) = self.declared_parent(class_name) {
            return Some(parent);
        }
        // Child-side metadata wins; `MemberClasses` fills the gap for ordinary
        // members whose outer class did not record an `InnerClass` entry.
        let stem = class_name.strip_suffix(';')?;
        let (outer, _) = stem.rsplit_once('$')?;
        let outer = format!("{outer};");
        self.nested_metadata(&outer)?;
        self.member_classes_of(&outer)
            .iter()
            .any(|child| child == class_name)
            .then_some(outer)
    }

    /// Source-level nested name recovered from `InnerClass`, when present.
    pub fn lexical_simple_name(&self, class_name: &str) -> Option<&str> {
        self.nested_metadata(class_name)?
            .inner_class
            .as_ref()?
            .name
            .as_deref()
    ...
    /// Class-level annotations of one class, decoded and cached on first use.
    fn nested_metadata(
        &self,
        class_name: &str,
    ...
    fn declared_parent(&self, class_name: &str) -> Option<String> {
        let metadata = self.nested_metadata(class_name)?;
        if let Some(parent) = metadata.enclosing_class.as_ref() {
            return Some(parent.clone());
        }
        if let Some(method) = metadata.enclosing_method.as_ref() {
            return method
                .split_once("->")
                .map(|(declaring, _)| declaring.to_string());
        }
        metadata.inner_class.as_ref()?.name.as_ref()?;
        let stem = class_name.strip_suffix(';')?;
        let (outer, _) = stem.rsplit_once('$')?;
        Some(format!("{outer};"))
    }

    fn member_classes_of(&self, class_name: &str) -> Vec<String> {
        self.nested_metadata(class_name)
            .map(|metadata| metadata.member_classes.clone())
            .unwrap_or_default()
    }

    /// Children of a class: the exact inverse of the global parent map.
    ///
    /// `declared_parent(C) == P`, plus `P`'s `MemberClasses` entries that declare
    /// no parent of their own. The first is global, so it cannot be recovered by
    /// scanning just `P`'s own metadata or the descriptors that share its name
    /// prefix; the whole map is built once on first use and cached.
    fn nested_children_index(&self) -> &HashMap<String, Vec<String>> {
        self.nested_children.get_or_init(|| {
            let mut parents: HashMap<String, String> = HashMap::new();
            for dex in &self.dex_files {
                for class in &dex.classes.items {
                    let Some(name) = class.class_name(&dex.types, &dex.strings) else {
                        continue;
                    };
                    if let Some(parent) = self.declared_parent(name) {
                        parents.insert(name.to_string(), parent);
                    }
                }
            }
            for dex in &self.dex_files {
                for class in &dex.classes.items {
                    ...
                    for child in self.member_classes_of(name) {
                        if !self.class_locations.contains_key(&child) {
                            continue;
                        }
                        if self.declared_parent(&child).is_none() {
                            parents.entry(child).or_insert_with(|| name.to_string());
                        }
                    }
                }
            }
    ...
    /// Children of a class, recovered from the shared index.
    fn children_of(&self, class_name: &str) -> Vec<String> {
        self.nested_children_index()
            .get(class_name)
            .cloned()
            .unwrap_or_default()
    }
    ...
    pub(crate) fn hierarchy_declarations(
        ...
        Item = (
            &rusty_dex::dex::file::DexFile,
            &rusty_dex::dex::classes::ClassDefItem,
        ),
    > {
        self.dex_files
            .iter()
            .flat_map(|dex| dex.classes.items.iter().map(move |class| (dex, class)))
    }
        ...
        for dex_idx in 0..self.dex_files.len() {
            ...
            for class_index in 0..class_count {
                let class_name = {
                    ...
                    let class = &dex.classes.items[class_index];
                    class
                        .class_name(&dex.types, &dex.strings)
                        .ok_or(DexError::InvalidTypeIndex(class.class_idx()))?
                        .to_string()
                };
    ...
    fn load_class_from_def(
        ...
        let class_name = class_def
            .class_name(&dex.types, &dex.strings)
            .ok_or(DexError::InvalidTypeIndex(class_def.class_idx()))?
            .to_string();
        ...
        if let Some(superclass) = class_def.superclass(&dex.types, &dex.strings) {
            class_node.set_super_class(parse_type(superclass)?);
        }
        for interface_descriptor in class_def.interfaces(&dex.types, &dex.strings) {
            class_node.add_interface(parse_type(interface_descriptor)?);
        }
        class_node.source_file = class_def.source_file(&dex.strings).map(str::to_string);
...
impl DexFileReader {
    ...
    fn link_inner_classes(&mut self) {
        ...
        let lexical = class_names
            .iter()
            .map(|class_name| {
                (
                    class_name.clone(),
                    self.lexical_parent(class_name),
                    self.children_of(class_name),
                )
            })
            .collect::<Vec<_>>();
        for (class_name, parent, children) in lexical {
            if let Some(class) = self.classes.get_mut(&class_name) {
    ...
    pub fn get_type(&self, dex_idx: usize, idx: u32) -> Option<&str> {
        if let Some(dex) = self.dex_files.get(dex_idx) {
            dex.types.descriptor(&dex.strings, idx)
        } else {
            None
        }
    }
    ...
    pub fn get_method(&self, dex_idx: usize, idx: u32) -> Option<Cow<'_, str>> {
        if let Some(dex) = self.dex_files.get(dex_idx) {
            dex.methods
                .render(&dex.types, &dex.protos, &dex.strings, idx)
                .map(Cow::Owned)
        } else {
            None
        }
    ...
    pub fn get_field(&self, dex_idx: usize, idx: u32) -> Option<Cow<'_, str>> {
        if let Some(dex) = self.dex_files.get(dex_idx) {
            dex.fields
                .render(&dex.types, &dex.strings, idx)
                .map(Cow::Owned)
        } else {
            None
        }
    ...
    match target {
        DexReferenceTarget::FieldName { class, name } => Some(
            dex.fields
                ...
                .iter()
                .enumerate()
                .filter_map(|(index, _)| dex.fields.render(&dex.types, &dex.strings, index as u32))
        ...
        DexReferenceTarget::MethodArity { class, name, arity } => Some(
            dex.methods
                ...
                .filter_map(|(index, _)| {
                    dex.methods
                        .render(&dex.types, &dex.protos, &dex.strings, index as u32)
                })
...
mod tests {
    ...
    // TEMPORARY equivalence check for the lazy lexical index.
    #[test]
    fn lazy_nested_index_matches_eager() {
        let Ok(path) = std::env::var("RASC_EQUIV_DEX") else {
            return;
        };
        let bytes = std::fs::read(&path).unwrap();
        let dex = rusty_dex::dex::file::DexFile::build(
            rusty_dex::dex::reader::DexReader::build(bytes.clone()).unwrap(),
        )
        .unwrap();
        let mut eager_parents = BTreeMap::new();
        let mut eager_simple = BTreeMap::new();
        ...
        for class in &dex.classes.items {
            ...
            if let Some(parent) = eager_declared_parent(class, &dex) {
                eager_parents.insert(name.clone(), parent);
            }
            if let Some(nested) = class
                .get_inner_class_annotation()
                .and_then(|inner| inner.name.clone())
            {
                eager_simple.insert(name, nested);
            }
        }
        for class in &dex.classes.items {
            let name = class
                ...
                .unwrap()
                .to_string();
            for child in class.get_member_classes() {
                if names.contains(child) {
                    eager_parents.entry(child.clone()).or_insert(name.clone());
                }
            }
        }
        let mut eager_children: BTreeMap<String, BTreeSet<String>> = BTreeMap::new();
        for (child, parent) in &eager_parents {
            eager_children
                .entry(parent.clone())
                .or_default()
                .insert(child.clone());
        ...
        let reader = DexFileReader::from_bytes(&bytes).unwrap();
        let mut diffs = 0usize;
        for name in &names {
            let lazy_parent = reader.lexical_parent(name);
            let eager_parent = eager_parents.get(name);
            if lazy_parent.as_ref() != eager_parent && diffs < 25 {
            ...
            if lazy_children != eager_children && diffs < 25 {
                ...
                eprintln!(
                    "children {name}: lazy={lazy_children:?} eager={eager_children:?} members={members:?}"
                );
                for child in &eager_children {
                    eprintln!(
                        "   child {child} declared_parent={:?}",
                        reader.declared_parent(child)
                    );
                }
                diffs += 1;
            }
        }
        assert_eq!(diffs, 0, "lazy lexical index diverged from the eager index");
    }

    fn eager_declared_parent(
        class: &rusty_dex::dex::classes::ClassDefItem,
        dex: &rusty_dex::dex::file::DexFile,
    ) -> Option<String> {
        ...
        class.get_inner_class_annotation()?.name.as_ref()?;
