...
use crate::language::kotlin::{KotlinConstructorLayout, KotlinIdentifier};
...
pub(in crate::analysis::kotlin_backend) struct EnclosingInstanceAbi {
...
/// Kotlin source-level representation of the implicit enclosing instance carried
/// by a non-static nested class in DEX ABI.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(in crate::analysis::kotlin_backend) struct OuterInstanceField {
...
impl OuterInstanceField {
    ...
    pub(in crate::analysis::kotlin_backend) fn analyze_cfgs<'a>(
        class: &ClassNode,
    ...
    pub(in crate::analysis::kotlin_backend) fn analyze(class: &ClassNode) -> Option<Self> {
...
/// A synthetic constructor added by the Kotlin compiler to bridge source-level
/// constructor access. Its trailing marker parameter does not exist in Kotlin
/// source and the bridge declaration itself is not a source member.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::analysis::kotlin_backend) struct SyntheticConstructorBridge {
...
impl ConstructorSourceAbi {
    ...
    fn layout(
        ...
        class: &ClassNode,
        constructor: &MethodNode,
    ) -> Option<KotlinConstructorLayout> {
        if self.hidden_parameters.is_empty() {
            return None;
        }
        let mut layout = KotlinConstructorLayout::new(
...
/// Computes the generic variables visible at each class declaration. Kotlin's
...
impl<'a> LexicalTypeEnvironment<'a> {
        ...
        let mut environment = Self {
            ...
            scopes: classes
                ...
                .map(|class| {
                    (
                        class.class_type().clone(),
                        KotlinSourceAbi::declared_type_parameters(class),
                    )
                })
                .collect(),
        };
    ...
    fn scope(&self, class: &ClassNode) -> Vec<TypeParameter> {
        let mut scope = class
            ...
            .unwrap_or_default();
        scope.extend(KotlinSourceAbi::declared_type_parameters(class));
        scope
    }
...
pub(crate) struct KotlinSourceAbi {
    constructors: Vec<KotlinConstructorLayout>,
    methods: Vec<MethodReference>,
    nullability: super::nullability::DexNullabilityContracts,
    ...
    mapped_collection_size_owners: BTreeSet<ArgType>,
...
impl KotlinSourceAbi {
    ...
    fn analyze_inner<'a, 'cfg>(
        ...
        let mark = |name: &str, started: crate::timing::Instant| {
            if stats {
                eprintln!(
        ...
        let mut abi = Self {
            ...
            generic_method_declarations,
            inaccessible_top_level_imports,
            mapped_collection_size_owners: BTreeSet::new(),
            generic_hierarchy,
            platform_generic_hierarchy,
            platform_symbols,
        };
        ...
        for class in &classes {
            if !class.is_interface()
                || !class
                    .interfaces
                    .iter()
                    .any(|parent| abi.is_subtype(parent, &iterable))
            {
                continue;
            }
            let has_size = class.methods().iter().any(|method| {
                method.name() == "size"
                    && method.param_types().is_empty()
                    && method.return_type() == &ArgType::INT
            });
            let has_indexed_get = class.methods().iter().any(|method| {
                method.name() == "get"
                    && method.param_types() == [ArgType::INT]
                    && method.return_type().is_reference()
            });
            if has_size && has_indexed_get {
                abi.mapped_collection_size_owners
                    .insert(class.class_type().clone());
            }
        }
        let collection = ArgType::object("java/util/Collection");
        for class in &classes {
            let inherits_collection = class
                .super_class
                .as_ref()
                .is_some_and(|parent| abi.is_subtype(parent, &collection));
            if !inherits_collection {
                continue;
            }
            for interface in &class.interfaces {
                if abi.methods.iter().any(|method| {
                    &method.owner == interface
                        && method.name == "size"
                        && method.descriptor.parameters.is_empty()
                        && method.descriptor.return_type == ArgType::INT
                }) {
                    abi.mapped_collection_size_owners.insert(interface.clone());
                }
            }
        }
        mark("tail", t);
    ...
    /// Members a Kotlin class declares under a name other than the JVM one.
    pub(crate) fn declared_source_names(
        &self,
    ) -> std::sync::Arc<BTreeMap<MethodReference, crate::language::kotlin::KotlinIdentifier>> {
        self.declared_members.source_names()
    }

    pub(crate) fn declared_property_getters(
        &self,
    ) -> std::sync::Arc<BTreeMap<MethodReference, crate::language::kotlin::KotlinIdentifier>> {
        self.declared_members.property_getters()
    }

    pub(crate) fn declared_property_setters(
        &self,
    ) -> std::sync::Arc<BTreeMap<MethodReference, crate::language::kotlin::KotlinIdentifier>> {
        self.declared_members.property_setters()
    }

    pub(crate) fn is_default_property_accessor(&self, method: &MethodReference) -> bool {
        self.declared_members.is_default_property_accessor(method)
    }

    pub(crate) fn declared_computed_property(
        &self,
        method: &MethodReference,
    ) -> Option<&crate::language::kotlin::KotlinIdentifier> {
        self.declared_members.computed_property(method)
    }

    pub(crate) fn is_sealed_class(&self, class: &ArgType) -> bool {
        self.declared_members.is_sealed(class)
    }

    pub(crate) fn is_constant_field(&self, field: &crate::ir::FieldReference) -> bool {
        self.declared_members.is_constant(field)
    }

    /// Kotlin classes with exactly one instance.
    pub(crate) fn singleton_types(&self) -> std::sync::Arc<BTreeSet<ArgType>> {
        self.declared_members.singletons()
    }

    pub(crate) fn singleton_instances(&self) -> std::sync::Arc<BTreeSet<FieldReference>> {
        self.declared_members.singleton_instances()
    }

    /// The visibility a Kotlin class declares for itself.
    pub(crate) fn declared_class_visibility(
        &self,
        class: &ArgType,
    ) -> Option<crate::frontend::kotlin_metadata::Visibility> {
        self.declared_members.class_visibility(class)
    }

    pub(crate) fn declared_class_kind(
        &self,
        class: &ArgType,
    ) -> Option<crate::frontend::kotlin_metadata::ClassKind> {
        self.declared_members.class_kind(class)
    }

    pub(crate) fn is_singleton_instance(&self, field: &FieldReference) -> bool {
        self.declared_members.is_singleton_instance(field)
    }

    /// The visibility a Kotlin class declares for one of its own members.
    pub(crate) fn declared_method_visibility(
        &self,
        method: &MethodReference,
    ) -> Option<crate::frontend::kotlin_metadata::Visibility> {
        self.declared_members.method_visibility(method)
    }

    pub(crate) fn declared_field_visibility(
        &self,
        field: &crate::ir::FieldReference,
    ) -> Option<crate::frontend::kotlin_metadata::Visibility> {
        self.declared_members.field_visibility(field)
    }

    /// The name the source gave one parameter of a Kotlin method.
    pub(crate) fn declared_parameter_name(
        &self,
        method: &MethodReference,
        parameter: usize,
    ) -> Option<&str> {
        self.declared_members.parameter_name(method, parameter)
    }
    ...
    pub(crate) fn declared_parameter_type(
        ...
        method: &MethodReference,
        parameter: usize,
    ) -> Option<&crate::frontend::kotlin_metadata::TypeReference> {
        self.declared_members.parameter_type(method, parameter)
    }
    ...
    pub(crate) fn declared_return_type(
        ...
        self.declared_members.return_type(method)
    }

    pub(crate) fn declared_vararg_element_nullable(
        &self,
        method: &MethodReference,
        parameter: usize,
    ) -> Option<bool> {
        self.declared_members
            .vararg_element_nullable(method, parameter)
    }

    pub(crate) fn declared_vararg_parameters(
        &self,
    ) -> std::sync::Arc<
        std::collections::BTreeMap<MethodReference, std::collections::BTreeSet<usize>>,
    > {
        self.declared_members.vararg_parameters()
    }

    pub(crate) fn declared_extension_receiver(&self, method: &MethodReference) -> Option<usize> {
        self.declared_members.extension_receiver(method)
    }

    pub(crate) fn declared_extension_receivers(
        &self,
    ) -> std::sync::Arc<std::collections::BTreeMap<MethodReference, usize>> {
        self.declared_members.extension_receivers()
    }

    pub(crate) fn declared_default_calls(
        &self,
    ) -> std::sync::Arc<
        ...
        >,
    > {
        self.declared_members.default_calls()
    }

    pub(crate) fn is_default_dispatcher(&self, method: &MethodReference) -> bool {
        self.declared_members.is_default_dispatcher(method)
    }

    pub(in crate::analysis::kotlin_backend) fn declared_default_arguments(
        &self,
        method: &MethodReference,
    ) -> Option<&super::declared_members::KotlinDefaultArgumentLayout> {
        self.declared_members.default_arguments(method)
    }

    pub(crate) fn declared_suspend_function(
        &self,
        method: &MethodReference,
    ) -> Option<&super::declared_members::KotlinSuspendDeclaration> {
        self.declared_members.suspend_declaration(method)
    }

    pub(crate) fn declared_source_types(
        &self,
        owner: &ArgType,
    ) -> impl Iterator<Item = ArgType> + '_ {
        self.declared_members.source_types_for(owner)
    }

    pub(crate) fn field_is_lateinit(&self, field: &crate::ir::FieldReference) -> bool {
        self.declared_members.field_is_lateinit(field)
    }

    pub(crate) fn field_is_mutable(&self, field: &crate::ir::FieldReference) -> bool {
        self.declared_members.field_is_mutable(field)
    }

    pub(crate) fn field_is_non_null(&self, field: &crate::ir::FieldReference) -> bool {
        self.nullability.field_is_non_null(field)
    }

    pub(crate) fn non_null_fields(&self) -> std::sync::Arc<BTreeSet<FieldReference>> {
        self.nullability.non_null_fields()
    }

    pub(crate) fn method_nullability(
        &self,
        method: &MethodReference,
    ) -> Option<&crate::language::kotlin::KotlinMethodNullability> {
        self.nullability.get(method)
    }

    pub(crate) fn analysis_dependencies(&self) -> impl Iterator<Item = &ArgType> {
        self.nullability.dependencies()
    }

    pub(crate) fn referenced_nullability<'a>(
        &'a self,
        methods: impl IntoIterator<Item = &'a MethodReference>,
    ) -> BTreeMap<MethodReference, crate::language::kotlin::KotlinMethodNullability> {
        self.nullability.select(methods)
    }

    pub(crate) fn is_mapped_collection_size_owner(&self, owner: &ArgType) -> bool {
        self.mapped_collection_size_owners.contains(owner)
    }

    pub(crate) fn platform_symbols(
        &self,
    ) -> Option<std::sync::Arc<crate::platform_symbols::PlatformSymbolSet>> {
        self.platform_symbols.clone()
    }

    pub(crate) fn symbolic_constant_owner_types<'a>(
        &self,
        methods: impl IntoIterator<Item = &'a MethodReference>,
    ) -> BTreeSet<ArgType> {
        ...
        methods
            ...
            .flat_map(|method| method.parameter_domains.iter().flatten())
            .flat_map(|domain| &domain.members)
            .filter_map(|member| member.field.owner.parse().ok())
            .collect()
    }

    pub(crate) fn import_is_accessible(
        &self,
        import: &crate::language::kotlin::KotlinClassName,
    ) -> bool {
        !self
            .inaccessible_top_level_imports
    ...
    pub(crate) fn inherited_member_type_names<'a>(
        &'a self,
        owner: &'a ArgType,
    ) -> impl Iterator<Item = &'a KotlinIdentifier> {
    ...
    pub(crate) fn is_subtype(&self, subtype: &ArgType, supertype: &ArgType) -> bool {
        subtype == supertype
            ...
            || self
                ...
                .as_ref()
                .is_some_and(|hierarchy| hierarchy.is_subtype(subtype, supertype))
            || self.is_platform_subtype(subtype, supertype)
    }

    fn is_platform_subtype(&self, subtype: &ArgType, supertype: &ArgType) -> bool {
    ...
    fn build_inherited_member_types(
        classes: &[&ClassNode],
    ) -> BTreeMap<ArgType, BTreeSet<(KotlinIdentifier, ArgType)>> {
        let by_type = classes
            .iter()
            .map(|class| (class.class_type().clone(), *class))
            .collect::<BTreeMap<_, _>>();
        let mut direct_members = BTreeMap::<ArgType, BTreeSet<(KotlinIdentifier, ArgType)>>::new();
