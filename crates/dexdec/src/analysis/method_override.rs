    ...
    let declarations = reader
        ...
        .map(|(dex, class)| {
            let class_name = class
                ...
                .ok_or_else(invalid_class_descriptor)?;
            let parents = class
                .superclass(&dex.types, &dex.strings)
                .into_iter()
                .chain(class.interfaces(&dex.types, &dex.strings))
                .map(hierarchy_object_name)
                .collect::<OverrideResult<Vec<_>>>()?;
            let access = AccessInfo::for_class(class.get_access_flags_raw());
            Ok((
                hierarchy_object_name(class_name)?,
                parents,
                ReferenceTypeInfo {
...
fn invalid_class_descriptor() -> OverrideAnalysisError {
    OverrideAnalysisError::PlatformSymbols(io::Error::new(
        io::ErrorKind::InvalidData,
        "class type index does not name a type",
    ))
}
