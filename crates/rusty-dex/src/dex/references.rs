//! Streaming traversal of symbolic references in DEX code items.
        ...
        while offset < insns_size {
            ...
            if let Some((kind, index)) = Self::reference_operand(&instruction) {
                ...
                let target = match kind {
                    DexReferenceKind::Type => self
                        .dex
                        .types
                        .descriptor(&self.dex.strings, index)
                        .map(str::to_string),
                    DexReferenceKind::Field => {
                        self.dex
                            .fields
                            .render(&self.dex.types, &self.dex.strings, index)
                    }
                    DexReferenceKind::Method => self.dex.methods.render(
                        &self.dex.types,
                        &self.dex.protos,
                        &self.dex.strings,
                        index,
