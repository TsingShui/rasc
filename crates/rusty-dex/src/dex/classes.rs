...
use crate::dex::strings::{DexStrings, StringIdx};
use crate::dex::types::{DexTypes, TypeIdx};
...
pub struct ClassDefItem {
    /// `type_ids` row naming this class. The descriptor text is resolved on
    /// demand through [`ClassDefItem::class_name`] so an unread class costs a
    /// few `u32`s at parse time.
    class_idx: TypeIdx,
    access_flags_raw: u32,
    access_flags: Vec<AccessFlag>,
    /// `type_ids` row naming the superclass, absent for `java/lang/Object`.
    superclass_idx: Option<TypeIdx>,
    interfaces_off: u32,
    /// Interface `type_ids` rows from the `type_list` at `interfaces_off`, in
    /// declaration order. Resolved on demand through
    /// [`ClassDefItem::interfaces`].
    interfaces: Vec<TypeIdx>,
    /// `string_ids` row naming the source file, when debug metadata records it.
    source_file_idx: Option<StringIdx>,
    annotations_off: u32,
    class_data_off: u32,
    ...
    /// Class-level Dalvik metadata decoded on first use when the class was
    /// parsed at [`ClassDecodeLevel::Declaration`]. See
    /// [`ClassDefItem::nested_metadata`].
    nested_metadata: OnceLock<ClassNestedMetadata>,
}

/// Lexical / nested-class metadata recovered from a class's class-level
/// annotations.
///
/// `build_metadata` does not decode class annotations, so callers that need
/// `InnerClass`, `MemberClasses`, `EnclosingClass`, `EnclosingMethod` or the
/// generic `Signature` go through [`DexFile::nested_metadata`] (or the
/// `ClassDefItem::*` accessors after materialization), which fills this once
/// per class. The fields mirror the materialized `ClassDefItem` fields of the
/// same names so both paths report the same text.
#[derive(Debug, Default, Clone)]
pub struct ClassNestedMetadata {
    pub signature: Option<String>,
    pub inner_class: Option<InnerClassAnnotation>,
    pub member_classes: Vec<String>,
    pub enclosing_class: Option<String>,
...
impl DexClasses {
    ...
    pub fn build_with_level(
        ...
        for _ in 0..size {
            ...
            // Keep the id indices; the descriptor and source-file text is
            // resolved on demand so an unread class is never decoded.
            ...
            // `type_idx: u16`.
            let interfaces = read_interface_list(dex_reader, interfaces_off, types_list)?;

            let superclass_idx = if superclass_idx == NO_INDEX {
                None
            } else {
                if types_list.items.get(superclass_idx as usize).is_none() {
                    return Err(DexError::InvalidTypeIdx);
                }
                Some(superclass_idx)
            };
            ...
            let source_file_idx = if source_file_idx == NO_INDEX {
                ...
                if strings_list.strings.get(source_file_idx as usize).is_none() {
            ...
            let annotations = if annotations_off != 0 && level != ClassDecodeLevel::Declaration {
            ...
            if level != ClassDecodeLevel::Declaration && class_data_off != 0 {
                ...
                for i in 0..static_fields_size {
                    ...
                    let decoded_field = fields_list
                        .render(types_list, strings_list, field_idx)
    ...
    pub fn get_class_def(
        ...
        self.items
            .iter()
            .find(|item| item.class_name(types, strings) == Some(class_name))
    }
}
...
/// Read a DEX `type_list` at `interfaces_off` and return each `type_ids` row.
/// Returns an empty vector when `interfaces_off == 0`.
fn read_interface_list(
    dex_reader: &mut DexReader,
    interfaces_off: u32,
    types_list: &DexTypes,
) -> Result<Vec<TypeIdx>, DexError> {
    if interfaces_off == 0 {
        return Ok(Vec::new());
    }
    ...
    for _ in 0..size {
        ...
        if types_list.items.get(type_idx as usize).is_none() {
            return Err(DexError::InvalidTypeIdx);
        }
...
/// Decode only a class's own annotations (its class-level annotation set).
/// Used by `DexFile::nested_metadata` when the class directory was parsed
/// without annotations.
pub(crate) fn read_class_annotations_directory(
    dex_reader: &mut DexReader,
    annotations_off: u32,
    types: &DexTypes,
    strings: &DexStrings,
    ...
    let class_annotations = if class_annotations_off == 0 {
        ...
        for _ in 0..size {
            ...
            let annotation_type = skip_encoded_annotation(dex_reader, types, strings)?;
            dex_reader.bytes.seek(SeekFrom::Start(current_offset))?;
...
impl ClassDefItem {
    ...
    /// Offset of this class's `annotations_directory_item`, or 0 when absent.
    pub fn annotations_offset(&self) -> u32 {
        self.annotations_off
    }

    /// The eagerly-decoded metadata when already present (materialized class or
    /// a previous [`DexFile::nested_metadata`] call).
    pub(crate) fn cached_nested_metadata(&self) -> Option<&ClassNestedMetadata> {
        self.nested_metadata.get()
    }

    /// Record lazily-decoded class-level metadata. Returns `false` when another
    /// thread won the race, in which case the stored value is authoritative.
    pub(crate) fn store_nested_metadata(&self, metadata: ClassNestedMetadata) -> bool {
        self.nested_metadata.set(metadata).is_ok()
    }

    /// Get the `type_ids` row naming this class.
    pub fn class_idx(&self) -> TypeIdx {
        self.class_idx
    }

    /// Resolve this class's descriptor (e.g. `Lcom/example/Foo;`).
    pub fn class_name<'a>(&self, types: &'a DexTypes, strings: &'a DexStrings) -> Option<&'a str> {
        types.descriptor(strings, self.class_idx)
    }
    ...
    /// Resolve the superclass descriptor, if this class declares one.
    pub fn superclass<'a>(&self, types: &'a DexTypes, strings: &'a DexStrings) -> Option<&'a str> {
        self.superclass_idx
            .and_then(|idx| types.descriptor(strings, idx))
    }

    /// Get the interface `type_ids` rows in declaration order.
    pub fn interface_indices(&self) -> &[TypeIdx] {
        &self.interfaces
    }

    /// Resolve the implemented interface descriptors (e.g.
    /// `Ljava/io/Serializable;`), in declaration order.
    pub fn interfaces<'a>(
        &'a self,
        types: &'a DexTypes,
        strings: &'a DexStrings,
    ) -> impl Iterator<Item = &'a str> {
        self.interfaces
            .iter()
            .filter_map(move |idx| types.descriptor(strings, *idx))
    }

    /// Resolve the source file name recorded in debug metadata, if present.
    pub fn source_file<'a>(&self, strings: &'a DexStrings) -> Option<&'a str> {
        self.source_file_idx
            .and_then(|idx| strings.strings.get(idx as usize))
