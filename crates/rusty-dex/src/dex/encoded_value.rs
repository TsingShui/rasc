...
impl EncodedValue {
    pub fn read(
        ...
        match value_type {
            ...
            0x15 => {
                let idx = read_unsigned(reader, value_arg, 4)? as u32;
                let value = protos
                    .and_then(|protos| protos.render(types, strings, idx))
            ...
            0x19 => {
                let idx = read_unsigned(reader, value_arg, 4)? as u32;
                let value = fields
                    .and_then(|fields| fields.render(types, strings, idx))
                    .ok_or(DexError::InvalidFieldIdx)?;
                Ok(Self::Field(value))
            }
            0x1a => {
                let idx = read_unsigned(reader, value_arg, 4)? as u32;
                let value = match (methods, protos) {
                    (Some(methods), Some(protos)) => methods.render(types, protos, strings, idx),
                    _ => None,
                }
...
mod tests {
    use super::*;
    use crate::dex::methods::MethodRow;
    use crate::dex::protos::ProtoRow;
    use std::io::Cursor;
    ...
    fn value_method_uses_method_id_table() {
        ...
        let strings = DexStrings {
            strings: vec!["Lpkg/Owner;".into(), "V".into(), "method".into()],
        };
        let types = DexTypes { items: vec![0, 1] };
        ...
        assert_eq!(
            value,
            EncodedValue::Method("Lpkg/Owner;->method()V".to_string())
        );
    }
