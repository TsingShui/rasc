pub fn decode(raw: &[u8]) -> Result<Vec<u16>, &'static str> {
    ...
    Decoder::new(raw).decode_each(|unit| output.push(unit))?;
    Ok(output)
}

/// Same validation as [`decode`], without materializing the UTF-16 units.
///
/// The parse path uses this to keep the declared-length check cheap: the units
/// are decoded later, on first access (see `DexString`).
pub fn decode_len(raw: &[u8]) -> Result<usize, &'static str> {
    let mut len = 0;
    Decoder::new(raw).decode_each(|_| len += 1)?;
    Ok(len)
}
...
impl<'a> Decoder<'a> {
    ...
    fn decode_each(mut self, mut sink: impl FnMut(u16)) -> Result<(), &'static str> {
        while let Some(lead) = self.next() {
            match lead {
                0x01..=0x7f => sink(u16::from(lead)),
                0xc0..=0xdf => {
